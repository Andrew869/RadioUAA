<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql = 'SELECT id_user, username, email, password_hash, rol, fecha_creacion, ultimo_acceso, cuenta_activa, sesion_activa FROM users WHERE username = :username LIMIT 1';
        ...
        if(!$user) echo "Nombre de usuario o contraseña incorrectos";
            ...
            if (hash_equals($user['password_hash'], hash('sha256', $password_input)) && $user['cuenta_activa'] && !$user['sesion_activa']) {
                ...
                $_SESSION['sesion_activa'] = $user['sesion_activa'];

                // modifica el ultimo acceso
                $sql = 'UPDATE users SET ultimo_acceso = :ultimo_acceso, sesion_activa = TRUE WHERE username = :username LIMIT 1';
                $stmt = $conn->prepare($sql);
